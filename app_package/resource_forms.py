...
class AddResourceForm(FlaskForm):
    res_name=StringField("Resource name: ",validators=[DataRequired(),NumberRange(min=10)])
    res_capacity=IntegerField("Capacity: ",validators=[DataRequired()])
...
class UpdateResourceForm(FlaskForm):
    ...
    res_capacity=IntegerField("new capacity: ",validators=[DataRequired(),NumberRange(min=10)])
    res_status=RadioField("Resource status: ",choices=[('available','available'),('inavailable','inavailable')])
    res_rent=IntegerField("Enter the rent : ",validators=[DataRequired(),NumberRange(min=1000)])
    type_of_use=StringField("Type of use : ",validators=[DataRequired()])
